...
export function AIChatPage({ session, onGoToReport, onStartNewAnalysis }: AIChatPageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-8rem)] flex flex-col">
      {/* Header Bar */}
      <div className="bg-white border border-gray-200 rounded-t-2xl p-4 flex items-center justify-between shrink-0 shadow-xs">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" icon={<FileText className="w-4 h-4" />} onClick={onGoToReport}>
            View Report
          </Button>
          <Button variant="ghost" size="sm" icon={<Plus className="w-4 h-4" />} onClick={onStartNewAnalysis}>
            New Analysis
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 bg-gray-50 border-x border-gray-200 p-6 overflow-y-auto space-y-4">
          ...
          >
            ...
            <div
              className={`max-w-lg rounded-2xl p-4 text-sm shadow-xs ${
                msg.sender === 'user'
                  ? 'bg-[#6D28D9] text-white rounded-tr-none font-medium'
        ...
        {loading && (
          <div className="flex items-center gap-3 text-xs text-gray-500 italic">
            <Sparkles className="w-4 h-4 text-[#6D28D9] animate-spin" />
      ...
      <div className="bg-white border border-gray-200 rounded-b-2xl p-4 shrink-0 shadow-xs">
        <form onSubmit={handleSend} className="flex items-center gap-3">
          <input
            ...
            className="flex-1 px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:border-[#6D28D9] focus:ring-1 focus:ring-[#6D28D9] disabled:bg-gray-100"
          />
          <Button
            type="submit"
